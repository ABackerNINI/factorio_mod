...
function(config_changed_data)
    ...
    math_ceil(
    startup_settings.default_cc_power_consumption * global.technologies.power_consumption_percentage
    ) or
    global.technologies.rc_power_consumption ~=
    math_ceil(
    startup_settings.default_rc_power_consumption * global.technologies.power_consumption_percentage
    )
    then
        ...
        if global.cc_entities.entities[index].nearest_lc ~= nil then
            entity.surface.create_entity {
                ...
                text = {
                    ...
                    string.format(
                    "%.1f",
                    calc_distance_between_two_points(
                    entity.position,
                    global.cc_entities.entities[index].nearest_lc.eei.position
                    )
                    )
                }
            }
        ...
        if global.rc_entities.entities[index].nearest_lc ~= nil then
            entity.surface.create_entity {
                ...
                text = {
                    ...
                    calc_distance_between_two_points(
                    entity.position,
                    global.rc_entities.entities[index].nearest_lc.eei.position
                    )
                    )
                }
            }
        ...
        for i = 1, 4 do
            if research.name == tech_lc_capacity_names[i] then
                ...
                if global.technologies.tech_lc_capacity_real_level < (i - 1) * 10 then
                    global.technologies.lc_capacity =                    config.default_lc_capacity + tech_lc_capacity_increment_sum[i] +
                    config.tech_lc_capacity_increment[i] *
                    (global.technologies.tech_lc_capacity_real_level - (i - 1)) *
                    10
                else
                    global.technologies.lc_capacity =                    config.default_lc_capacity + tech_lc_capacity_increment_sum[i] +
                    config.tech_lc_capacity_increment[i] *
                    (global.technologies.tech_lc_capacity_real_level - (i - 1) * 10)
                end
        ...
        for i = 1, 4 do
            if research.name == tech_power_consumption_names[i] then
                ...
                if global.technologies.tech_power_consumption_real_level < (i - 1) * 10 then -- fix with calling research_all_technologies() through Lua script
                    ...
                    config.tech_power_consumption_decrement[i] *
                    (global.technologies.tech_power_consumption_real_level - (i - 1)) *
                    10)
                else
                    power_consumption_percentage =                    1 -
                    (tech_power_consumption_decrement_sum[i] +
                    config.tech_power_consumption_decrement[i] *
                    (global.technologies.tech_power_consumption_real_level - (i - 1) * 10))
                end
